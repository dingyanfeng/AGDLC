...
# 创建保存结果的目录
data_path=/home/dyf/Dynamic-skmer/Result-om/embsize/${base_name_without_extension}
if [ ! -d "${data_path}" ]; then
  mkdir -p "${data_path}"
...
function DSKmer() {
  ...
  (/bin/time -v -p sh /home/dyf/Dynamic-skmer/compress-ems.sh ${data} ${T_SKLIST} ${T_MODULE} ${GPU} 32 ${T_EMS}) >${log} 2>&1

  if [ $? -ne 0 ]; then
    echo "compression task ERROR!"
    echo "/bin/time -v -p sh /home/dyf/Dynamic-skmer/compress-ems.sh ${data} ${T_SKLIST} ${T_MODULE} ${GPU} 32 ${T_EMS}"
    exit 1
  fi
  ...
  echo "${T_SKLIST}_${T_MODULE}_${T_EMS}, ${SourceFileSize}, ${CompressedFileSize}, ${CompressionRatio}, $(timer_reans $CompressionTime), ${CompressionMemory}" >>${sum_result}
...
for (( i = 8; i >= 1; i-- )); do
    EMB_I=$((i * 8))
    echo "DSKmer 2.2+2.3 xLSTM $EMB_I"
    DSKmer 2.2+2.3 xLSTM $EMB_I
done
