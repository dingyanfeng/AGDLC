...
# 创建保存结果的目录
data_path=/home/dyf/Dynamic-skmer/Result-om/batchsize/${base_name_without_extension}
if [ ! -d "${data_path}" ]; then
  mkdir -p "${data_path}"
...
echo "ALG, DS(B), CS(B), CR(bits/base), CT(S), CPM(KB)" >> ${sum_result}
# 时间转换函数
...
function DSKmer() {
  ...
  (/bin/time -v -p sh /home/dyf/Dynamic-skmer/compress-bs.sh ${data} ${T_SKLIST} ${T_MODULE} ${GPU} ${M_TIMESTEPS} ${T_BS}) >${log} 2>&1

  if [ $? -ne 0 ]; then
    echo "compression task ERROR!"
    echo "/bin/time -v -p sh /home/dyf/Dynamic-skmer/compress-bs.sh ${data} ${T_SKLIST} ${T_MODULE} ${GPU} ${M_TIMESTEPS} ${T_BS}"
    exit 1
  fi
  ...
  echo "${T_SKLIST}_${T_MODULE}_${T_BS}, ${SourceFileSize}, ${CompressedFileSize}, ${CompressionRatio}, $(timer_reans $CompressionTime), ${CompressionMemory}" >>${sum_result}
...
for (( i = 1; i <= 16; i++ )); do
    BS_I=$((i * 32))
    echo "DSKmer 2.2+2.3 xLSTM $BS_I"
    DSKmer 2.2+2.3 xLSTM $BS_I
done
